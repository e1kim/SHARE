...
from generation import model_response, response_session_prompt, response_notag_prompt
...
def episode(data, backbone, update_method, outputfilename, total_session_number, start_num, end_num, batch_size=2):
    ...
    for session_number in range(2, total_session_number):
        ...
        if backbone == "llama" and (update_method == "update" or update_method == "accumulate"):
            gen_path = "chano12/llama3_with_tag"
            gen_model, gen_tokenizer = get_peft_llama(gen_path, device)

        elif backbone == "llama" and update_method == "noshare":
            gen_path = "chano12/llama3_noshare_tag"
            gen_model, gen_tokenizer = get_peft_llama(gen_path, device)

        elif backbone == "gemma" and (update_method == "update" or update_method == "accumulate"):
            gen_path = "chano12/gemma_with_tag"
            gen_model, gen_tokenizer = get_peft_gemma(gen_path, device)

        elif backbone == "gemma" and update_method == "noshare":
